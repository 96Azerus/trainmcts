...
try:
    ...
    class PlayerBoard: TOTAL_CAPACITY = 13; ROW_NAMES = ['top', 'middle', 'bottom']; ROW_CAPACITY = {'top':3,'middle':5,'bottom':5}; def __init__(self, r=None,c=0): self.rows=r or {n:[] for n in PlayerBoard.ROW_NAMES}; self._cards_placed=c; def copy(self): return self; pass # type: ignore
    class Card: @staticmethod # type: ignore
    def get_rank_int(c): return 0; @staticmethod # type: ignore
    def get_suit_int(c): return 0; @staticmethod # type: ignore
    def to_str(c): return "??" ; pass # type: ignore
    class Deck: FULL_DECK_CARDS=set(); def __init__(self,c=None):pass; def deal(self,n):return[]; def get_remaining_cards(self):return[];pass # type: ignore
...
RAVE_K: float = 500.0  # Константа для RAVE
PW_C: float = 2.0      # Константа C для Progressive Widening
PW_ALPHA: float = 0.5  # Константа alpha для Progressive Widening

MAX_PERMUTATIONS_STREET_1: int = 120 # 5!
MAX_PERMUTATIONS_SLOTS_STREET_1: int = 30 # Ограничение для P(13,5), чтобы не было слишком много
MAX_PERMUTATIONS_STREET_N: int = 20 # Для 2-3 карт, P(N,k)

HEURISTIC_FOUL_PENALTY = -1000.0 # Глобальный штраф за фол, используется в роллаутах
...
class MCTSNode:
    ...
    def _get_dynamic_weights(cards_placed_on_board: int, num_unknown_removed: int) -> Dict[str, float]:
        weights = {
            ...
            'fantasy_aa_bonus_abs': 50.0, 'fantasy_trips_bonus_abs': 70.0, # Увеличены бонусы
            'fantasy_draw_multiplier_vs_abs_bonus': 0.15, # Множитель для бонуса ФЛ при дро (1 карта на топе)
            'fantasy_draw_multiplier_vs_abs_bonus_2cards': 0.08, # Множитель для бонуса ФЛ при дро (2 карты на топе)
            ...
            'almost_foul_penalty': -15.0, # Увеличен штраф за "почти фол"

            'made_flush_score': 80.0, 'flush_draw_score_per_out': 2.8, # Немного увеличен
            'three_to_flush_score_per_out': 1.2, 'sf_bonus_over_flush': 110.0, # Увеличен
            'made_straight_score': 70.0, 'open_ended_draw_score_per_out': 2.2, # Немного увеличен
            'gutshot_draw_score_per_out': 1.1,
            'three_to_open_ended_score_per_out': 0.9, 'three_to_gutshot_score_per_out': 0.5,
            
            'four_of_a_kind_score': 160.0, 'three_of_a_kind_made_score': 45.0, # Увеличены
            'two_pair_made_score': 22.0, 'pair_made_score': 6.0, # Увеличены
            'pair_made_score_4cards': 3.0,
            
            'out_to_quads_score': 12.0, 'out_to_trips_score': 3.5, # Увеличены
            ...
            'royalty_base_multiplier': 2.5, # Увеличен множитель для роялти
        ...
        if progress < 0.35:  # Ранняя игра (0-4 карты)
        ...
        elif progress < 0.65: # Средняя игра (5-8 карт)
        ...
        if num_unknown_removed > 0:
            uncertainty_factor = max(0.3, 1.0 - (num_unknown_removed * 0.12)) # Уменьшение на 12% за "?"
    ...
    def _generate_next_states(self, cards_just_dealt: List[int]) -> List[Tuple[PlayerBoard, Any]]:
        ...
        else: # Последующие улицы (2-5)
            # По правилам Ананаса, если сдано 3, ставим 2, 1 сброс.
            # Если это последний ход и мест меньше 2, ставим сколько есть.
            if num_dealt == 3:
                num_to_place_on_board = min(2, available_slots_count)
            elif num_dealt > 0 : # Если сдано не 3 (например, в режиме ФЛ или спец. ситуации)
                num_to_place_on_board = min(num_dealt -1 if num_dealt > 1 else num_dealt, available_slots_count)
            else: # num_dealt == 0
                return []
        
        if num_to_place_on_board <= 0 and num_dealt > 0: # Нечего ставить, но карты есть -> сброс всех
    ...
    def uct_select_child(self, exploration_constant: float) -> Optional['MCTSNode']:
        ...
        parent_visits_log = math.log(self.visits) if self.visits > 1 else 0 # log(1)=0, избегаем log(0)

        for child_node in self.children.values():
            if child_node.visits == 0: logger.error("UCT: Child with 0 visits after FPU. Should not happen."); return child_node # Защита
            ...
            rave_q_value = 0.0
            alpha_rave = 0.0 # По умолчанию RAVE не используется
            if child_node.rave_visits_count > 0:
                # Простая alpha, уменьшающаяся с количеством визитов UCB
            ...
            else: # Нет RAVE статистики для этого ребенка
                score = q_value + ucb_exploration_term
    ...
    def _calculate_heuristic_score_v2(board: PlayerBoard, deck_snapshot: Set[int], is_first_street: bool = False, num_unknown_removed: int = 0, original_deck_size_for_snapshot: int = 0) -> float:
        ...
        cards_on_b = board.get_total_cards()
        factor = 1.0
    ...
    def _is_straight_flush_possible(card_ranks: List[int], card_suits: List[int], target_suit: Optional[int]) -> Tuple[bool, List[int]]:
        ...
        if len(set(s_ranks)) < num_s : return False, s_ranks # Пары внутри
        is_s = (max(s_ranks) - min(s_ranks) == num_s - 1)
        is_w = False
    ...
    def _calculate_straight_potential_for_row(ranks: List[int], num_cards: int, deck: Set[int], num_unknown_removed: int, original_deck_size: int, weights: Dict[str, float]) -> float:
        ...
        if num_cards == 4 and len(unique_ranks) == 4:
            ...
            # Broadway OESD (TJQK needs A or 9) - T=8, J=9, Q=10, K=11. Needs A(12) or 9(7)
            ...
            if outs == 0: # Check Gutshot if not OESD
                gut_outs = 0
    ...
    def _calculate_n_of_a_kind_potential(ranks: List[int], rank_counts: Counter, num_cards: int, deck: Set[int], num_unknown_removed: int, original_deck_size: int, weights: Dict[str, float]) -> float:
        score = 0.0 # ... (логика как в предыдущем ответе, но все константы заменены на weights['...'])
        if num_cards == 4:
            ...
            if trips_r!=-1: score+=weights['three_of_a_kind_made_score']
                ...
                if k_r!=-1: def c_kp(c):return Card.get_rank_int(c)==k_r; score+=MCTSNode._count_specific_outs(deck,c_kp,num_unknown_removed,original_deck_size)*weights['out_to_pair_score'] # Simplified for 3 cards
            else: # 3 unique
                for r_v in ranks: def c_p(c):return Card.get_rank_int(c)==r_v; score+=MCTSNode._count_specific_outs(deck,c_p,num_unknown_removed,original_deck_size)*weights['out_to_pair_score_needing_two']
        return score
    ...
    def _choose_best_heuristic_placement_v2(
        ...
        if num_to_place_on_board == 0 and num_dealt > 0: # Только сброс
        ...
        if num_to_place_on_board == num_dealt: # Все сданные карты размещаются
            cards_to_place_options = [cards_to_act_on]; cards_to_discard_options = [None]
        elif num_dealt > num_to_place_on_board: # Нужно выбрать, какие разместить, какие сбросить
            ...
            for combo_to_place_tuple in itertools.combinations(cards_to_act_on, num_to_place_on_board):
                ...
                else: cards_to_discard_options.append(None) # Не должно быть, если num_to_discard > 0
        else: # num_dealt < num_to_place_on_board - невозможная ситуация по логике выше
            logger.error(f"Heuristic: num_dealt ({num_dealt}) < num_to_place ({num_to_place_on_board}). Should not happen."); return []
        ...
        for i in range(len(cards_to_place_options)):
            current_cards_to_place_list = cards_to_place_options[i]
            current_discard_info = cards_to_discard_options[i]
            ...
            actual_slots_to_fill = min(num_to_place_on_board, len(available_slots)) # На случай, если available_slots < num_to_place
            if actual_slots_to_fill < num_to_place_on_board : continue # Не можем разместить нужное количество
            ...
            for p_cards_tuple_perm in card_perms:
                ...
                for p_slots_tuple_perm in slot_perms:
                    temp_board = current_board.copy(); placements_list: List[Tuple[int, str, int]] = []
                    valid_placement = True; deck_after_action = current_deck.copy()
                    try:
                        for card_idx, (card_val, (row_val, slot_idx_val)) in enumerate(zip(p_cards_list_perm, p_slots_tuple_perm)):
                            ...
                            if card_val in deck_after_action: deck_after_action.remove(card_val)
                        if not valid_placement: continue
                        if is_first_street: # Правило трипса на топе
                            rc_fs = Counter(Card.get_rank_int(c) for c in p_cards_list_perm)
                            tr_fs = next((r for r,c in rc_fs.items() if c >=3), -1)
                        ...
                        if current_discard_info is not None:
                            if isinstance(current_discard_info, tuple):
                                for dc in current_discard_info:
                                    if dc in deck_after_action: deck_after_action.remove(dc)
        ...
        return candidate_actions[:(15 if not is_first_street else 10)] # Возвращаем топ N эвристических ходов
...
def heuristic_rollout_simulation_v2(board_dict: Dict, deck_list_initial: List[int], num_unknown_sim: int) -> Tuple[float, List[Dict[str, Any]]]:
    ...
    for r, c_strs in board_dict.get('rows', {}).items():
        for i, c_str in enumerate(c_strs):
            if c_str and c_str != CARD_PLACEHOLDER: try: current_board.add_card(Card.from_str(c_str), r, i)
            except ValueError: pass

    deck_for_sim: Set[int]
    if num_unknown_sim > 0 and len(deck_list_initial) > num_unknown_sim:
        try: deck_for_sim = set(random.sample(deck_list_initial, len(deck_list_initial) - num_unknown_sim))
        except ValueError: deck_for_sim = set(deck_list_initial)
    elif num_unknown_sim > 0: deck_for_sim = set()
    else: deck_for_sim = set(deck_list_initial)
    
    sim_deck_obj = Deck(cards=deck_for_sim)
    actions_hist: List[Dict[str, Any]] = []
    try:
        while not current_board.is_complete():
            ...
            n_deal, n_place = (5,5) if current_board.get_total_cards()==0 else (3, min(2, avail_slots))
            if n_place == 0: break
            if len(sim_deck_obj) < n_deal: break
            dealt = sim_deck_obj.deal(n_deal)
            if not dealt: break
            
            # Для _choose_best_heuristic_placement_v2, current_deck - это то, что осталось в sim_deck_obj *после* deal
            # original_deck_size_for_snapshot - это размер current_deck + len(dealt)
