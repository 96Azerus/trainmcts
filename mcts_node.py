...
# ИСПРАВЛЕНО: Учет сброшенных карт в MCTSNode.expand (Проблема №3)
# ИСПРАВЛЕНО: Полная логика для "колеса" в _detect_straight_draw
# ИЗМЕНЕНО: Увеличены бонусы за Фантазию
"""
...
try:
    ...
    STR_RANKS = ""; RANK_QUEEN = 10; RANK_KING = 11; RANK_ACE = 12
...
ROW_PAIR_OUTS_WEIGHT = 0.4
ROW_TRIPS_OUTS_WEIGHT = 0.5
ROW_HIGH_CARD_WEIGHT = 0.01
...
class MCTSNode:
    ...
    def expand(self) -> Optional['MCTSNode']:
        ...
        try:
            ...
            # Удаляем из колоды ребенка карты, которые были размещены в этом ходе
            if placement_info_for_child and placement_info_for_child.get('placements'):
                for p_card, _, _ in placement_info_for_child['placements']:
                    ...
                    # else:
                        # logger.debug(f"Expand: Placed card {Card.to_str(p_card)} "
                        #              f"not found in parent's deck for child. Parent deck size: {len(self.remaining_deck)}")

            # Удаляем из колоды ребенка карту, которая была сброшена в этом ходе
            if discarded_card_from_generation is not None:
                ...
                # else:
                    # logger.debug(f"Expand: Discarded card {Card.to_str(discarded_card_from_generation)} "
                    #              f"not found in parent's deck for child. Parent deck size: {len(self.remaining_deck)}")
    ...
    def _detect_straight_draw(cards: List[int]) -> Tuple[int, Set[int]]:
        ...
        # Числовые значения рангов 0-12 (2=0, ..., A=12)
        ...
        for start_rank_val in range(ACE_R, THREE_R, -1): # От A-хай до 5-хай (A5432 уже покрыто колесом)
            potential_straight_ranks = set(range(start_rank_val - 4, start_rank_val + 1))
            # Исключаем пересечение с уже найденными аутами для колеса, если это колесо-подобный стрит
            # Это предотвратит двойной подсчет, например, для A2345 (колесо) и 23456 (обычный стрит)
            # if ACE_R in potential_straight_ranks and TWO_R in potential_straight_ranks and THREE_R in potential_straight_ranks and FOUR_R in potential_straight_ranks and FIVE_R in potential_straight_ranks:
            #     if draw_type > 0 and ACE_R in needed_ranks: # Если уже нашли дро на колесо с тузом
            #         continue
    ...
    @staticmethod
    def _estimate_row_potential(row_name: str, row_cards: List[int], board_cards_after: Set[int], remaining_deck: Set[int]) -> float:
        ...
        if s_type > 0:
            ...
            if s_type == 2: potential_score += s_outs * ROW_STRAIGHT_DRAW_OUT_WEIGHT
            elif s_type == 1: potential_score += s_outs * ROW_GUTSHOT_DRAW_OUT_WEIGHT
        ranks = [Card.get_rank_int(c) for c in row_cards]; rank_counts = Counter(ranks)
        for r, count in rank_counts.items():
            if count == 2:
                pair_outs = len({c for c in remaining_deck if Card.get_rank_int(c) == r and c not in board_cards_after})
                potential_score += pair_outs * ROW_PAIR_OUTS_WEIGHT
            elif count == 3:
                trips_outs = len({c for c in remaining_deck if Card.get_rank_int(c) == r and c not in board_cards_after})
        ...
        for row_name in PlayerBoard.ROW_NAMES:
            ...
            if num_cards_in_row == row_capacity:
                ...
                if rank != WORST_RANK:
                    ...
                    elif row_name == "bottom" and hand_class <= 2: current_row_score += HEURISTIC_FL_REPEAT_BONUS
            else:
    ...
    def _choose_best_heuristic_placement_v2(board: PlayerBoard, cards_dealt: List[int], remaining_deck: Set[int]) -> Optional[Dict[str, Any]]:
        ...
        if best_placement_info is None and evaluated_count > 0:
             logger.warning(f"Heuristic v2.7: No valid (non-foul) placement found. Evaluated {evaluated_count}."); return None
            ...
            while not current_board.is_complete():
                ...
                for card, row, idx in placements:
                    if not current_board.add_card(card, row, idx):
                        logger.error(f"Heuristic v2.7: Failed to apply placement {Card.to_str(card)}@{row}[{idx}]"); valid_placement = False; break
