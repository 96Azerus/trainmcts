# mcts_node.py v2.8 (Enhanced Evaluation Logic)
...
Использует ПРОДВИНУТУЮ ЭВРИСТИЧЕСКУЮ симуляцию v2.8:
...
try:
    ...
    from ofc_evaluators import (
        ...
        evaluate_3_card_ofc, HAND_TYPE_TRIPS_3, HAND_TYPE_PAIR_3, # Добавлены типы для 3 карт
        evaluator_5card
    )
except ImportError as e:
    logging.critical(f"Failed to import from ofc_logic/ofc_evaluators in mcts_node.py: {e}")
    # Заглушки ... (оставлены для краткости)
    class PlayerBoard:
        ...
        def add_card(self, c, r, i): return False; def get_row_cards(self, rn): return [];_
        def get_all_cards(self): return set(); def get_available_slots(self): return []
        def get_total_cards(self): return 0; def is_complete(self): return False
        def get_board_state_tuple(self): return tuple(); def copy(self): return PlayerBoard()
    class Card:
        @staticmethod
        def get_rank_int(c): return 0; @staticmethod
        def get_suit_int(c): return 0; @staticmethod
        def hand_to_int(cs): return []; @staticmethod
        def hand_to_str(ci): return []; @staticmethod
        def to_str(c): return "??"
    class Deck: FULL_DECK_CARDS = set()
    def get_row_royalty(*args): return 0; def check_board_foul(*args): return False
...
# --- Новые константы для v2.8 ---
FIRST_STREET_STRONG_HAND_ON_BOTTOM_BONUS = 50.0 # Бонус за сильную руку на боттоме на 1й улице
BOTTOM_ROW_PAIR_BONUS = 2.0 # Небольшой бонус за пару на боттоме
EMPTY_BOTTOM_PENALTY = -5.0 # Штраф за пустой низ, если можно было положить пару
FANTASY_SUPPORT_OUT_WEIGHT = 0.7 # Вес для аутов на мидле, поддерживающих Фантазию

# Веса для оценки рядов в эвристике
ROW_COMPLETED_HAND_WEIGHT = 1.0
ROW_FLUSH_DRAW_OUT_WEIGHT = 0.8 # Базовый вес
ROW_STRAIGHT_DRAW_OUT_WEIGHT = 0.6 # Базовый вес
ROW_GUTSHOT_DRAW_OUT_WEIGHT = 0.3
ROW_PAIR_OUTS_WEIGHT = 0.5 # Немного увеличен
ROW_TRIPS_OUTS_WEIGHT = 0.6 # Немного увеличен
ROW_HIGH_CARD_WEIGHT = 0.005 # Немного уменьшен
...
class MCTSNode:
    # ... ( __init__, is_terminal, _generate_next_states, expand - без изменений по сравнению с v2.7 + фикс для expand) ...
    def __init__(self,
                 board: PlayerBoard,
    ...
    def _detect_straight_draw(cards: List[int]) -> Tuple[int, Set[int]]:
        # ... (полная логика для колеса и обычных стритов из предыдущего ответа) ...
        ...
        ACE_R = 12; TWO_R = 0; THREE_R = 1; FOUR_R = 2; FIVE_R = 3; SIX_R = 4;
    ...
    def _estimate_row_potential(row_name: str, row_cards: List[int],
                                ...
                                top_row_for_fl_check: Optional[List[int]] = None) -> float: # Добавлен top_row_for_fl_check
        ...
        if num_cards_in_row == row_capacity: # Завершенный ряд
            rank, _, _ = get_hand_rank_safe(row_cards)
            if rank != WORST_RANK: potential_score = (WORST_RANK - rank) * 0.1 # Базовая сила
            # Дополнительный бонус за пару на боттоме (если это боттом и это пара)
            if row_name == "bottom":
                _, hand_class_bot, _ = get_hand_rank_safe(row_cards)
                if hand_class_bot == 8: # Пара
        ...
        if f_suit is not None:
            ...
            # Увеличиваем вес флеш-дро для боттома
        ...
        if s_type > 0:
            ...
            # Увеличиваем вес стрит-дро для боттома
        ...
        # e) Потенциал поддержки Фантазии для мидла
        if row_name == "middle" and top_row_for_fl_check and len(top_row_for_fl_check) > 0:
            # Определяем целевой ранг для Фантазии на топе
            top_ranks_int = [Card.get_rank_int(c) for c in top_row_for_fl_check]
            potential_fl_rank_on_top = -1
            if any(r == RANK_QUEEN for r in top_ranks_int): potential_fl_rank_on_top = RANK_QUEEN
            if any(r == RANK_KING for r in top_ranks_int): potential_fl_rank_on_top = max(potential_fl_rank_on_top, RANK_KING)
            if any(r == RANK_ACE for r in top_ranks_int): potential_fl_rank_on_top = max(potential_fl_rank_on_top, RANK_ACE)
            
            if potential_fl_rank_on_top != -1:
                # Ищем ауты на мидле для пар СТАРШЕ чем potential_fl_rank_on_top
                for mid_card_rank_int in ranks: # Карты уже на мидле
                    if mid_card_rank_int > potential_fl_rank_on_top:
                        # Ауты на пару этой карты
                        support_outs = len({c for c in remaining_deck if Card.get_rank_int(c) == mid_card_rank_int and c not in board_cards_after})
                        potential_score += support_outs * FANTASY_SUPPORT_OUT_WEIGHT
                # Также можно добавить ауты на новые карты, которые могут прийти на мидл и образовать нужную пару
                # Это сложнее, пока ограничимся улучшением существующих карт на мидле.
    ...
    def _score_placement_v2(board: PlayerBoard, placement_info: Dict[str, Any], remaining_deck: Set[int]) -> float:
        ...
        initial_cards_on_board = board.get_total_cards() # Для определения улицы
        ...
        # Получаем состояние топа для передачи в _estimate_row_potential мидла
            ...
            if num_cards_in_row == row_capacity: # Завершенный ряд
                ...
                if rank != WORST_RANK:
                    ...
                    if row_name == "top":
                        ...
                        if hand_class <= 2: current_row_score += HEURISTIC_FL_REPEAT_BONUS # Каре+ на боттоме
                        elif hand_class == 8: current_row_score += BOTTOM_ROW_PAIR_BONUS # Бонус за пару на боттоме
                    elif row_name == "middle" and hand_class == 6: # Сет на мидле
                        current_row_score += HEURISTIC_FL_REPEAT_BONUS # Повтор Фантазии за сет на мидле (если правила это позволяют)
                                                                    # По стандартным правилам - нет, но можно оставить как эвристику силы
            else: # Незавершенный ряд
        ...
        # "Экспертное" правило для первой улицы
        if is_first_street:
            bottom_row_cards = temp_board.get_row_cards("bottom")
            if len(bottom_row_cards) == PlayerBoard.ROW_CAPACITY['bottom']: # Если боттом заполнен (5 карт)
                _, class_b_first_street, _ = get_hand_rank_safe(bottom_row_cards)
                if class_b_first_street <= 6: # Трипс или лучше на боттоме
                    score += FIRST_STREET_STRONG_HAND_ON_BOTTOM_BONUS
        
        # Штраф за пустой низ, если можно было положить пару
        placed_cards_in_move = [p[0] for p in placements]
        if not temp_board.get_row_cards("bottom") and \
           (temp_board.get_row_cards("middle") or temp_board.get_row_cards("top")):
            # Проверяем, была ли пара среди размещаемых карт
            ranks_in_placed_cards = Counter(Card.get_rank_int(c) for c in placed_cards_in_move)
            had_pair_to_place_on_bottom = any(count >= 2 for count in ranks_in_placed_cards.values())
            if had_pair_to_place_on_bottom:
                score += EMPTY_BOTTOM_PENALTY
                
        if is_fl_qualify_hand_placed: # Продвинутая проверка безопасности Fantasyland
    ...
    def _choose_best_heuristic_placement_v2(board: PlayerBoard, cards_dealt: List[int], remaining_deck: Set[int]) -> Optional[Dict[str, Any]]:
        ...
        generated_count = 0; evaluated_count = 0; MAX_PERMUTATIONS = 24; MAX_SLOT_COMBINATIONS = 100
        for cards_to_place, discarded_card in cards_to_place_options:
            ...
            for card_permutation in permutations(cards_to_place):
                ...
                for slot_combination in combinations(available_slots, num_to_place):
                    ...
                    generated_count += 1; current_placements: List[Tuple[int, str, int]] = []
                    ...
                    if score >= best_score:
                        if score > HEURISTIC_FOUL_PENALTY + 1: best_score = score; best_placement_info = placement_info
        if best_placement_info is None and evaluated_count > 0:
             logger.warning(f"Heuristic v2.8: No valid (non-foul) placement found. Evaluated {evaluated_count}. Best score was {best_score}"); return None
    ...
    def heuristic_rollout_simulation_v2(initial_board: PlayerBoard, initial_remaining_deck: Set[int]) -> Tuple[float, List[Dict[str, Any]]]:
        ...
        try:
            ...
            while not current_board.is_complete():
                ...
                for card, row, idx in placements:
                    if not current_board.add_card(card, row, idx):
                        logger.error(f"Heuristic v2.8: Failed to apply placement {Card.to_str(card)}@{row}[{idx}]"); valid_placement = False; break
            ...
            logger.error(f"Error during heuristic rollout simulation v2.8: {e}", exc_info=True)
