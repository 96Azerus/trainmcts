# mcts_node.py v2.8.4 (CARD_PLACEHOLDER fix)
...
try:
    # ИСПРАВЛЕНО: Добавляем CARD_PLACEHOLDER в импорт из ofc_logic
    ...
    CARD_PLACEHOLDER = "__" # Заглушка для случая ошибки импорта
...
class MCTSNode:
    ...
    def expand(self) -> Optional['MCTSNode']:
        ...
        # Remove the expanded state from untried_next_states if it was there
        # This logic might need adjustment if untried_next_states is populated differently
        if self.untried_next_states:
            self.untried_next_states = [
                (b, d) for (b, d) in self.untried_next_states
                if (b, d) != (board_state, discarded_card) # Simple comparison, might need deep compare for board
            ]
        return child_node
...
def heuristic_rollout_simulation_v2(board_dict: Dict, deck_list: List[int]) -> Tuple[float, List[Dict[str, Any]]]:
    ...
    for r, cards_str_list in board_dict.get('rows', {}).items():
        for i, card_str_val in enumerate(cards_str_list):
            # ИСПРАВЛЕНО: Используем импортированную константу CARD_PLACEHOLDER
            if card_str_val and card_str_val != CARD_PLACEHOLDER:
                try: current_board.add_card(Card.from_str(card_str_val), r, i)
                except ValueError: pass
    deck_sim = Deck(cards=deck_list[:])
    random.shuffle(deck_sim.cards)
    ...
    try:
        while not current_board.is_complete():
            ...
            if len(deck_sim.cards) < num_to_deal: break
