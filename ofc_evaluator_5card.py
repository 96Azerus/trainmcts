# ofc_evaluator_5card.py v2.0 (Logging level consistency)
"""
...
try:
    ...
    class Card: # type: ignore
        ...
        @staticmethod
        def to_str(c): return "??"
    PRIMES = Card.PRIMES # type: ignore
    INT_RANKS = Card.INT_RANKS # type: ignore
...
class LookupTable5Card:
    ...
    def __init__(self):
        ...
        try:
            ...
            rf_bits = 0b1111100000000 # A, K, Q, J, T
            rf_prime = self._prime_product_from_rankbits(rf_bits)
            if rf_prime not in self.flush_lookup or self.flush_lookup[rf_prime] != 1:
                 logger.error(f"FATAL: Royal Flush prime {rf_prime} (rank {self.flush_lookup.get(rf_prime)}) incorrect!")
            else: logger.debug(f"Royal Flush prime {rf_prime} OK (rank 1).")
        except Exception as e: logger.critical(f"Error during 5-card table calculation: {e}", exc_info=True); raise

    def _calculate_flushes(self):
        # Bits for A, K, Q, J, T, 9, 8, 7, 6, 5, 4, 3, 2 (INT_RANKS order)
        straight_flushes_rank_bits: List[int] = [
            0b1111100000000, # AKQJT (Ace high)
            ...
            0b1000000001111, # A5432 (Steel Wheel)
        ]
        ...
        # Flushes are sorted from strongest to weakest (higher bits = stronger high cards)
        normal_flush_rank_bits = sorted([rb for rb in all_flush_rank_bits if rb not in straight_flush_set], reverse=True)
        
        rank = 1 # Strongest SF is rank 1
        for sf_bits in straight_flushes_rank_bits: # Already sorted strongest to weakest
        ...
        rank = self.MAX_FULL_HOUSE + 1 # Flushes start after Full Houses
    ...
    def _calculate_straights_and_highcards(self, straights_rank_bits: List[int], highcards_rank_bits: List[int]):
        # Straights (use the same rank_bits as straight flushes, but go into unsuited_lookup)
        rank = self.MAX_FLUSH + 1 # Straights start after Flushes
        for s_bits in straights_rank_bits: # Already sorted strongest to weakest
        ...
        # High Cards (use the same rank_bits as normal flushes, but go into unsuited_lookup)
        rank = self.MAX_PAIR + 1 # High Cards start after Pairs
        for h_bits in highcards_rank_bits: # Already sorted strongest to weakest
    ...
    def _calculate_multiples(self):
        backwards_ranks = range(len(INT_RANKS) - 1, -1, -1) # A, K, Q, ..., 2
        
        # Four of a Kind
        rank = self.MAX_STRAIGHT_FLUSH + 1 # Quads start after SFs
        for quad_idx in backwards_ranks: # Strongest quad rank first
            kickers = sorted([k_idx for k_idx in backwards_ranks if k_idx != quad_idx], reverse=True) # Strongest kicker first
        ...
        # Full House
        rank = self.MAX_FOUR_OF_A_KIND + 1 # FHs start after Quads
        for trip_idx in backwards_ranks: # Strongest trip rank first
            pair_options = sorted([p_idx for p_idx in backwards_ranks if p_idx != trip_idx], reverse=True) # Strongest pair rank first
        ...
        # Three of a Kind
        rank = self.MAX_STRAIGHT + 1 # Trips start after Straights
        for trip_idx in backwards_ranks:
            kickers = [k_idx for k_idx in backwards_ranks if k_idx != trip_idx]
            # Sort kicker combos: (K,Q) > (K,J) > (Q,J) etc.
            kicker_combos = sorted(itertools.combinations(kickers, 2), key=lambda x: (x[0], x[1]), reverse=True)
        ...
        # Sort pair combos: (A,K) > (A,Q) > (K,Q) etc.
        pair_combos = sorted(itertools.combinations(backwards_ranks, 2), key=lambda x: (x[0], x[1]), reverse=True)
        for p1, p2 in pair_combos: # p1 is higher pair rank
            kickers = sorted([k_idx for k_idx in backwards_ranks if k_idx != p1 and k_idx != p2], reverse=True) # Strongest kicker first
            for kick_idx in kickers:
        ...
        for pair_idx in backwards_ranks:
            kickers = [k_idx for k_idx in backwards_ranks if k_idx != pair_idx]
            # Sort kicker combos: (A,K,Q) > (A,K,J) etc.
            kicker_combos = sorted(itertools.combinations(kickers, 3), key=lambda x: (x[0], x[1], x[2]), reverse=True)
    ...
    def _prime_product_from_rankbits(self, rankbits: int) -> int:
        product = 1
        for i in INT_RANKS: # 0 to 12
            if rankbits & (1 << i): # Check if i-th bit is set
                try: product *= PRIMES[i]
                except IndexError: logger.error(f"Prime index {i} out of bounds for bits {bin(rankbits)}."); return 0
        return product

class Evaluator5Card:
    def __init__(self): self.table = LookupTable5Card()
    
    def evaluate(self, cards: List[int]) -> int: # Принимает список int карт
        ...
        if len(cards) != 5:
            ...
            raise ValueError(f"Requires 5 cards, got {len(cards)}") # Изменено на ValueError
        
        valid_cards: List[int] = []
        try:
            for c_int in cards:
                 if not (isinstance(c_int, int) and c_int != INVALID_CARD and c_int > 0): raise ValueError(f"Invalid card integer: {c_int}")
                 valid_cards.append(c_int)
            if len(valid_cards) != len(set(valid_cards)): raise ValueError(f"Duplicate cards found: {hand_str_log}")
        except ValueError as e: logger.warning(f"Invalid input for 5-card evaluation: {e}"); raise e

        ranks_extracted = [Card.get_rank_int(card_int) for card_int in valid_cards]
        
        # Проверка на флеш: все 5 карт имеют одинаковую масть
        # suit_int_first = Card.get_suit_int(valid_cards[0])
        # is_flush = all(Card.get_suit_int(c) == suit_int_first for c in valid_cards[1:])
        # Более надежный способ через битовую маску:
        suit_mask = valid_cards[0] & valid_cards[1] & valid_cards[2] & valid_cards[3] & valid_cards[4] & 0xF000 # 0xF000 это маска для битов масти

        if suit_mask != 0: # Все масти одинаковы
            logger.debug(f"Hand {hand_str_log} detected as Flush/SF (suit_mask={hex(suit_mask)})")
            rank_bitmask = 0
            ...
            if rank is None: logger.warning(f"Flush prime {prime_product} not found for {hand_str_log} (bits {bin(rank_bitmask)})"); return self.table.WORST_RANK_5CARD
            logger.debug(f"Returning rank (flush/sf): {rank}"); return rank
        else: # Не флеш
            logger.debug(f"Hand {hand_str_log} detected as Unsuited (or multiple suits)")
            prime_product = 1
            try:
                # Используем Counter для подсчета рангов, чтобы правильно вычислить произведение простых чисел
                # Например, для AA K Q J: A^2 * K^1 * Q^1 * J^1
                rank_counts = Counter(ranks_extracted)
                for rank_idx, count_of_rank in rank_counts.items():
                    prime_product *= PRIMES[rank_idx] ** count_of_rank
            except Exception as e: logger.error(f"Error calculating prime product for unsuited hand {hand_str_log}: {e}"); return self.table.WORST_RANK_5CARD
            
            rank = self.table.unsuited_lookup.get(prime_product)
            if rank is None: logger.warning(f"Unsuited prime {prime_product} not found for {hand_str_log} (ranks: {ranks_extracted})"); return self.table.WORST_RANK_5CARD
            logger.debug(f"Returning rank (unsuited): {rank}"); return rank

    def get_rank_class(self, hand_rank: int) -> int:
        if not (isinstance(hand_rank, int) and 0 < hand_rank < self.table.WORST_RANK_5CARD):
            logger.debug(f"get_rank_class invalid rank: {hand_rank}, returning 9 (High Card class)")
            return 9 
...
try:
...
except Exception as e_global_eval:
    logger.critical(f"Failed to create global Evaluator5Card instance: {e_global_eval}", exc_info=True)
    class MockEvaluator5CardGlobal: # type: ignore
        ...
        table = MockEvaluator5CardGlobal() 
