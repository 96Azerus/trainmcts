...
class MCTSAgent:
            ...
            while time.time() - start_mcts_time < self.time_limit:
                ...
                if not node_to_rollout_from.is_terminal():
                    try:
                        ...
                        if pool: # Параллельные роллауты
                             async_results = [pool.apply_async(run_parallel_rollout, task) for task in rollout_tasks]
                             for res in async_results:
                                  try:
                                       timeout_get = max(5.0, self.time_limit * 0.5) # Таймаут для получения результата
    ...
    def _select(self, node: MCTSNode) -> Tuple[List[MCTSNode], Optional[MCTSNode]]:
        ...
        while True:
            ...
            if current_node.untried_actions is None: # Инициализация при первом посещении
            ...
            if not current_node.children: return path, current_node # Лист без действий

            selected_child = current_node.uct_select_child(self.exploration)
            if selected_child is None:
                logger.warning(f"UCT selection returned None for node {current_node}. Parent visits: {current_node.visits}.")
    ...
    def _select_best_action(self,
        ...
        for best_action, visits, avg_reward in action_stats:
            if is_first_street and trip_in_hand_rank != -1:
                ...
                if Card.get_rank_int(card_int) == trip_in_hand_rank and row_name == 'top':
                    logger.warning(f"Rule Violation: Skipping action {self._format_action(best_action)} (Trip {Card.to_str(card_int)[0]} on Top on Street 1).")
                    continue # Пропускаем это действие

            # Если действие не нарушает правило (или правило не применяется), выбираем его
            logger.info(f"Selected action (Visits={visits}, AvgRoyalty={avg_reward:.2f}): {self._format_action(best_action)}")
            return best_action
